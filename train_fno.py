...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        epoch_train_loss = 0
        if epoch > args.warmup_epochs and steps_forward < 3: # make sure we don't go over 10 steps forward
            steps_forward += 0
        ...
        for i, batch in enumerate(pbar):
            ...
            batch_preds = pressures.shape[1] - steps_forward
            num_forward_pred += batch_preds
            batch_loss_sum = 0.0
            batch_scaled_loss_sum = 0.0
            for t in range(pressures.shape[1]-steps_forward):
                current_pressure = pressures[:, t, :, :].unsqueeze(1)
                next_pressures = pressures[:, t+steps_forward, :, :].unsqueeze(1)
                current_time = times[:, t]
                next_time = times[:, t+steps_forward]
                ...
                batch_scaled_loss_sum += scaled_loss.item()

                epoch_train_loss += loss.item()

            current_lr = optimizer.param_groups[0]['lr']
            avg_batch_loss = batch_loss_sum / batch_preds
            avg_scaled_loss = batch_scaled_loss_sum / batch_preds
            ...
            logger.add_scalar(f"loss: {epoch}", avg_batch_loss, global_step=epoch * l + i)
            logger.add_scalar("learning_rate", current_lr, global_step=epoch * l + i)

        epoch_train_loss /= num_forward_pred
        ...
        with torch.no_grad():
            ...
            for j, val_batch in enumerate(validation_dataloader):
                ...
                val_batch_preds = val_pressures.shape[1] - steps_forward
                val_forward_pred += val_batch_preds
                for t in range(val_pressures.shape[1]-steps_forward):
                    val_current_pressure = val_pressures[:, t, :, :].unsqueeze(1)
                    val_next_pressures = val_pressures[:, t+steps_forward, :, :].unsqueeze(1)
                    val_next_time = val_times[:, t+steps_forward]
            ...
            epoch_val_loss = eval_model_loss / num_forward_pred
...
def launch():
    ...
    parser.add_argument('--run_name', type=str, default="fno_ultra_low_res_home")
