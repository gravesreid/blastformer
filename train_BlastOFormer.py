...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(pbar):
            ...
            mape_loss = MAPE_error(prediction, y)
            loss.backward()
            #scaled_loss.backward()
            #l2_loss.backward()
            #huber_loss.backward()
            optimizer.step()
            pbar.set_postfix({"Loss": loss.item(), "scaled_loss": scaled_loss.item(), "l2_loss": l2_loss.item(), "huber_loss": huber_loss.item()})
            wandb.log({"train/loss": loss.item(), "train/scaled_loss": scaled_loss.item(), "train/l2_loss": l2_loss.item(), "train/huber_loss": huber_loss.item()})
